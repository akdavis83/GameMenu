    ...
    enemies.forEach(function(enemy, index) {
        if (Math.abs(player.x - enemy.x) > 400 && Math.abs(player.y - enemy.y) > 400) {
            ...
            if (enemy.health <= 0) {
                ...
                playerHealth = Math.min(playerHealth += 100, 10000000000000000000000000000000000000);  // Reward player with health
                playerStats.attack += 10;  // Increase player attack power
            }
        }
    });
}
...
function updateEnemyBehavior(enemy) {
    ...
    if (distance > 50) {
        ...
        playerHealth -= 5;  // Drain player health if too close
    }
}
