...
// Create player
var player = new ent();
player.sprite("https://opengameart.org/sites/default/files/lpcmalegoldenarmorpreview.png", 0, 640, 64, 64, 100, 100, 62, 62, 50);
    ...
    enemies.forEach(function(enemy, index) {
        if (Math.abs(player.x - enemy.x) > 400 && Math.abs(player.y - enemy.y) > 400) {
            ...
            if (enemy.health <= 0) {
                ...
                playerStats.attack += 100;  // Increase player attack power
            }
        }
    });
}
...
setInterval(function() {
    enemies.forEach(function(enemy) {
        enemy.health += 100;  // Increase health
