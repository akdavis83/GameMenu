...
window.requestAnimFrame = (function(){
  ...
  function createEnemy(x, y, health) {
    ...
    enemy.sprite(randomType, 0, 0, 88, 88, x, y, 128, 128, 50);
    enemy.load = true;
    enemy.health = 200000000;
    enemies.push(enemy);
  }
      ...
      enemies.forEach(function(enemy, index) {
          if (Math.abs(player.x - enemy.x) > 500 && Math.abs(player.y - enemy.y) > 500) {
              ...
              if (enemy.health <= 0) {
                  ...
                  playerHealth = Math.min(playerHealth += 1000000, 10000000000000000000000000000000000000);  // Reward player with health
                  playerStats.attack += 100000;  // Increase player attack power
              }
          }
      });
  }
  ...
  setInterval(function() {
      enemies.forEach(function(enemy) {
          enemy.health += 200000;  // Increase health
  ...
  function updateEnemyBehavior(enemy) {
      ...
      if (distance > 50) {
          ...
          playerHealth -= 10000;  // Drain player health if too close
      }
  }
      ...
      enemies.forEach(function(enemy, index) {
          if (Math.abs(player.x - enemy.x) < 500 && Math.abs(player.y - enemy.y) < 500) {
              ...
              if (enemy.health <= 0) {
                  ...
                  playerHealth = Math.min(playerHealth + 1000000, 100000000000000000000000000000000000000);
                  playerStats.attack += 100000;
