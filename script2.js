...
window.requestAnimFrame = (function(){
  ...
  function checkEndGame() {
      if (Math.abs(player.x - endGameLocation.x) < 64 && Math.abs(player.y - endGameLocation.y) < 64) {
          window.location.href = "index4.html"; // Redirect to next page
      }
  }
  
  // Create player
  var player = new ent();
  player.sprite("https://opengameart.org/sites/default/files/lpcmalegoldenarmorpreview.png", 0, 640, 64, 64, 100, 100, 62, 62, 15);
  ...
  function createEnemy(x, y, health) {
    ...
    var enemyTypes = [
        ...
        "./golem-walk.png"               // Example sprite 4
    ];
    var randomType = enemyTypes[Math.floor(Math.random() * enemyTypes.length)];
  
    enemy.sprite(randomType, 0, 0, 128, 128, x, y, 128, 128, 10);
    enemy.load = true;
    enemy.health = 35000;
    enemies.push(enemy);
  }
      ...
      enemies.forEach(function(enemy, index) {
          if (Math.abs(player.x - enemy.x) > 500 && Math.abs(player.y - enemy.y) > 500) {
              ...
              if (enemy.health <= 0) {
                  ...
                  playerHealth = Math.min(playerHealth += 1000, 10000000000000000000000000000000000000);  // Reward player with health
                  playerStats.attack += 1000;  // Increase player attack power
              }
          }
      });
  }
  ...
  setInterval(function() {
      enemies.forEach(function(enemy) {
          enemy.health += 1000;  // Increase health
  ...
  function updateEnemyBehavior(enemy) {
      ...
      if (distance > 50) {
          ...
          playerHealth -= 50;  // Drain player health if too close
      }
  }
      ...
      enemies.forEach(function(enemy, index) {
          if (Math.abs(player.x - enemy.x) < 500 && Math.abs(player.y - enemy.y) < 500) {
              ...
              if (enemy.health <= 0) {
                  ...
                  playerHealth = Math.min(playerHealth + 1000, 100000000000000000000000000000000000000);
                  playerStats.attack += 1000;
