...
window.requestAnimFrame = (function(){
  ...
  function checkEndGame() {
      if (Math.abs(player.x - endGameLocation.x) < 64 && Math.abs(player.y - endGameLocation.y) < 64) {
          window.location.href = "index6.html"; // Redirect to next page
      }
  }
      ...
      enemies.forEach(function(enemy, index) {
          if (Math.abs(player.x - enemy.x) > 500 && Math.abs(player.y - enemy.y) > 500) {
              ...
              if (enemy.health <= 0) {
                  ...
                  playerHealth = Math.min(playerHealth += 2000, 10000000000000000000000000000000000000);  // Reward player with health
                  playerStats.attack += 2000;  // Increase player attack power
              }
          }
      });
  }
  ...
  setInterval(function() {
      enemies.forEach(function(enemy) {
          enemy.health += 2000;  // Increase health
  ...
  function updateEnemyBehavior(enemy) {
      ...
      if (distance > 50) {
          ...
          playerHealth -= 500;  // Drain player health if too close
      }
  }
      ...
      enemies.forEach(function(enemy, index) {
          if (Math.abs(player.x - enemy.x) < 500 && Math.abs(player.y - enemy.y) < 500) {
              ...
              if (enemy.health <= 0) {
                  ...
                  playerHealth = Math.min(playerHealth + 2000, 100000000000000000000000000000000000000);
                  playerStats.attack += 2000;
