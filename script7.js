...
window.requestAnimFrame = (function(){
  ...
  function checkEndGame() {
      if (Math.abs(player.x - endGameLocation.x) < 64 && Math.abs(player.y - endGameLocation.y) < 64) {
          window.location.href = "index9.html"; // Redirect to next page
      }
  }
      ...
      enemies.forEach(function(enemy, index) {
          if (Math.abs(player.x - enemy.x) > 500 && Math.abs(player.y - enemy.y) > 500) {
              ...
              if (enemy.health <= 0) {
                  ...
                  playerHealth = Math.min(playerHealth += 100000, 10000000000000000000000000000000000000);  // Reward player with health
                  playerStats.attack += 10000;  // Increase player attack power
              }
          }
      });
  }
  ...
  setInterval(function() {
      enemies.forEach(function(enemy) {
          enemy.health += 10000;  // Increase health
      ...
      enemies.forEach(function(enemy, index) {
          if (Math.abs(player.x - enemy.x) < 500 && Math.abs(player.y - enemy.y) < 500) {
              ...
              if (enemy.health <= 0) {
                  ...
                  playerHealth = Math.min(playerHealth + 100000, 100000000000000000000000000000000000000);
                  playerStats.attack += 10000;
